...
- 已有可玩玩法：狼人杀、谁是卧底、海龟汤。内容矩阵仍预留迷案共研、暗号潜伏、默契连线、立场辩局、真相二选一，均为 `COMING_SOON`，只展示入口和定位，不允许创建房间。
...
- 首个真正落地的新增游戏为海龟汤：它与 LLM 的语言推理、主持、追问、线索控制能力匹配度最高。后续内容矩阵按好友组局场景扩展迷案、暗号、默契、辩论和破冰类玩法。
...
### 海龟汤 MVP 落地（2026-05-30）

- `turtle_soup` 已从内容矩阵预留升级为 `ACTIVE` 玩法，接入独立 `TurtleSoupGameEngine` 和前端 `TurtleSoupRoom`。
- MVP 使用后端内置精选题库，开局后公开汤面，汤底只在结算阶段通过 `GameState.extra.solution` 揭晓。
- 统一 `/action` 新增 `ASK_QUESTION` 和 `FINAL_GUESS`，玩家轮流向 AI 主持提问，AI 玩家会基于关键线索主动补问。
- AI 主持采用“规则判定兜底 + AI 润色”策略；模型不可用时仍返回固定的“是 / 否 / 不重要 / 接近但不准确”。
- 其他规划玩法仍为 `COMING_SOON`：`mystery_case`、`secret_signal`、`mind_match`、`debate_arena`、`truth_or_bluff`。
